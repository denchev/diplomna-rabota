...
CACHE_PATH = Path(__file__).resolve().parent / 'article_sentiment_cache.json'
...
df = pd.read_csv('./output/izbori/dir_bg.csv')
...
for url in unique_urls:
    ...
    article_content = soup.select_one('.article-content .article-body') # fakti.bg: .news-text, dnes.bg: .article-content, mediapool.bg: .c-text.c-article-content, dir.bg: .article-content .article-body
    article_title = soup.select_one('.article-main-section h1.title') # fakti.bg: .news-title, dnes.bg: .article-header h1, mediapool.bg: .c-heading.c-heading_size_1.c-heading_spaced, dir.bg: .article-main-section h1.title
...
for url, data in url_content.items():
    ...
    if sentiment is None:
        sentiment = {'sentiment': None, 'confidence': None, 'dominant_emotion': None, 'article_title': None}
...
for index, row in df.iterrows():
    ...
    comment = row['comment']
    article_sentiment = sentiment_results.get(article_url, {'sentiment': None, 'confidence': None, 'dominant_emotion': None, 'article_title': None, 'article_title_sentiment': None, 'article_title_confidence': None, 'article_title_dominant_emotion': None})
...
output_df.to_csv('output/izbori/dir_bg_with_news_data.csv', encoding='utf-8-sig', index_label='comment_text')