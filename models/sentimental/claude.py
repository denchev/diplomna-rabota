...
CACHE_PATH = Path(__file__).resolve().parent / 'comment_sentiment_claude_cache.json'
...
def analyze_bulgarian_sentiment(text_content):
    ...
    if cleaned_output.startswith("```json"):
        cleaned_output = cleaned_output[7:]  # Премахва "```json"
    if cleaned_output.endswith("```"):
        cleaned_output = cleaned_output[:-3]  # Премахва крайните "
...
output_df.to_csv('output/svobodno_vreme/dnes_bg_claude.csv', encoding='utf-8-sig')
print("\nАнализът завърши! Резултатите са записани в 'output/svobodno_vreme/dnes_bg_claude.csv'")
